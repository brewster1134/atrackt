    ...
    return Atrackt = (function() {
      ...
      Atrackt.prototype.track = function(data, options, event, context) {
        ...
        trackPlugins = (function(_this) {
          return function() {
            var eventNamespace, pluginData, pluginName, _ref, _ref1, _results;
            _ref = _this.plugins;
            _results = [];
            for (pluginName in _ref) {
              pluginData = _ref[pluginName];
              if (eventNamespace = event != null ? (_ref1 = event.handleObj) != null ? _ref1.namespace : void 0 : void 0) {
                if (eventNamespace === 'atrackt' || eventNamespace === ("atrackt." + pluginName)) {
                  _results.push(_this._trackJqueryObject(pluginData, data, options, event));
                } else {
                ...
                if (!options['_plugin'] || options['_plugin'] === pluginName) {
                  if (data instanceof jQuery) {
                    ...
                    _results.push(_this._track(pluginData, data, options, event));
                  }
                } else {
                  _results.push(void 0);
                }
