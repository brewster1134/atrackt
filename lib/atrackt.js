    ...
    Atrackt = function () {
      class Atrackt {
        ...
        setPlugin(pluginName, plugin) {
          ...
          plugin.setEvent = eventsObject => {
            return this.setEvent(eventsObject, plugin);
          };
          plugin.setData = data => {
            return this.setData(data, plugin);
          };
          plugin.setOptions = options => {
            return this.setOptions(options, plugin);
          };
          plugin.setCallback = (name, callback) => {
            return this.setCallback(name, callback, plugin);
          };
          return plugin.track = (data, options, event, plugin) => {
            return this.track(data, options, event, plugin);
          };
        }
        ...
        setEvent(eventsObject, context = this) {
          ...
          for (eventType in eventsObject) {
            ...
            results.push(function () {
              ...
              for (i = 0, len = objects.length; i < len; i++) {
                object = objects[i];
                results1.push($(object).each((index, element) => {
                  var base, base1, globalIndex, pluginData, pluginIndex, pluginName, ref, ref1, ref2, results2;
                  (base = this._elements)[eventType] || (base[eventType] = []);
                  ...
                  if (context.name) {
                    globalIndex = this._elements[eventType].indexOf(element);
                    ...
                    if (globalIndex === -1) {
                      ...
                      if (context._elements[eventType].indexOf(element) === -1) {
                        return this._registerElement(context, element, eventType);
                      }
                    }
                    // if binding globally
                    // ...and the element has not already been bound globally
                  } else if (this._elements[eventType].indexOf(element) === -1) {
                    this._registerElement(context, element, eventType);
                    ref = this.plugins;
            ...
            for (pluginName in ref) {
              ...
              if (eventNamespace = event != null ? (ref1 = event.handleObj) != null ? ref1.namespace : void 0 : void 0) {
                if (eventNamespace === 'atrackt' || eventNamespace === `atrackt.${pluginName}`) {
                  results.push(this._trackJqueryObject(pluginData, data, options, event));
                } else {
                  results.push(void 0);
                }
              } else {
                if (!options['_plugin'] || options['_plugin'] === pluginName) {
                  ...
                  if (data instanceof jQuery) {
                    ...
                    results.push(this._track(pluginData, data, options, event));
                  }
                } else {
                  results.push(void 0);
                }
        ...
        _trackJqueryObject(plugin, data, options, event) {
          ...
          return $(data).each((index, element) => {
            return this._track(plugin, element, options, event);
          });
        }
